# Custom Exit Codes
# 0 = Success (Deletion initiated and confirmed complete)
# 1001 = Prerequisite Missing (Module not installed OR wrong version)
# 1002 = Connection Failed (M365/Security & Compliance) 
# 1003 = Search Failed to Create/Start
# 1004 = Purge Action Failed to Create/Start
# 1005 = Loop Limit Exceeded (Likely due to persistent throttling/limits)
...
# FIX APPLIED: Increased Max Loops to ensure all contacts are removed despite throttling.
$MaxPurgeLoops = 10 
...
$RequiredModuleVersion = [version]"3.9.0"
...
try {
    ...
    if ($Module.Version -lt $RequiredModuleVersion) {
        Write-Log -Message "ERROR: EXO Module version $($Module.Version) found. Required minimum version is $($RequiredModuleVersion). Exiting." -Type "ERROR"
        exit 1001
    }
    Import-Module ExchangeOnlineManagement -ErrorAction Stop
    Write-Log -Message "ExchangeOnlineManagement module version $($Module.Version) loaded."
}
catch {
    exit 1000
}
...
try {
    Write-Log -Message "Connecting to Security & Compliance Center using Search-Only Session..."
    $Session = Connect-IPPSSession -EnableSearchOnlySession -ErrorAction Stop 
    Write-Log -Message "Successfully established Search-Only SCC session."
}
catch {
...
try {
    do {
        $PurgeLoopCount++
        Write-Log -Message "Starting Purge Loop $PurgeLoopCount of $MaxPurgeLoops..."
        ...
        try {
            ...
            if ($SearchStats.Items -ne $null) {
                # This counts total items found across all mailboxes
                $ItemsFound = $SearchStats.Items.Count 
            }
        } catch {
             Write-Log -Message "WARNING: Could not reliably determine ItemsFound count. Assuming at least 1 item for safety." -Type "WARNING"
             $ItemsFound = 1 
        }

        Write-Log -Message "Search completed. Found $ItemsFound total items matching criteria."

        if ($ItemsFound -eq 0) {
            Write-Log -Message "SUCCESS: No items remain after $PurgeLoopCount loops. Contact removal is complete." -Type "SUCCESS"
            break 
        }
