...
# 1001 = System already clean
# 1002 = Directory creation failed (Logging issue)
...
if (-not (Test-Path $LogPath)) {
    try { New-Item -ItemType Directory -Path $LogPath -Force -ErrorAction Stop | Out-Null } catch { exit 1002 }
}
...
try {
    ...
    foreach ($RegPath in $RegPaths) {
        if (Test-Path $RegPath) {
            Write-ToLogFile "Purging Registry: $RegPath"
    ...
    if (Test-Path $ProgramFilesPath) {
        Write-ToLogFile "Deleting Program Files: $ProgramFilesPath"
