...
.NOTES
    Script name: Intune-Configure-iOS-Apps-Microsoft-Graph-PowerShell.ps1
    Version:     1.10.0
    Author:      Resilient IT
    ...
    This script uses interactive delegated authentication by default.
#>
...
$LogPath = Join-Path -Path $LogDirectory -ChildPath "Intune-iOS-App-Deployment-$Timestamp.log"
$CsvPath = Join-Path -Path $LogDirectory -ChildPath "Intune-iOS-App-Deployment-$Timestamp.csv"
...
try {
    ...
    if ($null -eq $context -or -not $hasRequiredScope) {
        Write-Log -Message "Connecting to Microsoft Graph with scope $RequiredGraphScope."
        Connect-MgGraph -Scopes $RequiredGraphScope -NoWelcome -ErrorAction Stop
        $context = Get-MgContext
    }
    else {
