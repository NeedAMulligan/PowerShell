...
#
# might need to add this so powershell uses tls 1.2 [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
#
##################################################################################################################
...
foreach ($user in $users) {
    ...
    $firstname,
    <br><br>
    Your computer password (Email, CostPoint, VPN, Active Directory) for your <b>$sName</b> account $messageDays.  After it expires, you might have issues logging in until your password has been changed.
    <br><br>
    In order to change your password, you need to be at a corporate office or connected to the VPN to change your password. On a Windows machine you can press <b>Ctrl-Alt-Del</b> and select <b>Change Password</b>.
    <br><br>
    Thank you,
    <br><br>
    EWA Helpdesk
